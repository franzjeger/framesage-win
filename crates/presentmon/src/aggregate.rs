...
pub struct FrameStats {
    ...
    /// Reserved: PresentMon's `Dropped` column wiring is a follow-up;
    /// always 0 for now (honest absence, not a claim of zero drops).
...
impl FrameAggregator {
    ...
    /// Push one present. Returns a completed bucket's stats when
    /// `at_ms` crosses into a new second.
    pub fn push(&mut self, at_ms: u64, frame_time_us: u64) -> Option<FrameStats> {
...
mod tests {
    ...
    fn one_bucket_per_second_with_percentiles() {
        ...
        for i in 0..59 {
            assert_eq!(agg.push(i * 16, 16_000), None);
        }
        assert_eq!(agg.push(950, 40_000), None);
        // First frame of second 1 closes the bucket.
        let stats = agg.push(1_005, 16_000).expect("bucket emitted");
    ...
    fn empty_seconds_emit_nothing() {
        ...
        let stats = agg.push(5_000, 16_000).unwrap();
